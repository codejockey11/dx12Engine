...
CVideoDevice::CVideoDevice(CErrorLog* errorLog, CWindow* window)
{
	memset(this, 0x00, sizeof(CVideoDevice));
...
void CVideoDevice::DXGIFactory()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::DXGIFactory::CreateDXGIFactory2:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
}
...
void CVideoDevice::GetHardwareAdapter()
{
	for (UINT ai = 0; ; ai++)
	{
		...
		HRESULT hr = D3D12CreateDevice(m_adapter.Get(), D3D_FEATURE_LEVEL::D3D_FEATURE_LEVEL_12_1, _uuidof(ID3D12Device), 0);

		// hr == 1 means supported instead of using S_FALSE
		if (hr == 1)
		{
			m_errorLog->WriteError("CVideoDevice::D3D_FEATURE_LEVEL::D3D_FEATURE_LEVEL_12_1 supported\n");
...
void CVideoDevice::CreateDevice()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D12CreateDevice:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());

		return;
	}

	m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D12_FEATURE_DATA_SHADER_MODEL\n");

	D3D12_FEATURE_DATA_SHADER_MODEL shaderModel;
	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_5_1;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_5_1\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_0;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_0\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_1;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_1\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_2;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_2\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_3;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_3\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_4;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_4\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_5;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_5\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_6;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_6\n");
	}

	shaderModel.HighestShaderModel = D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_7;
	hr = m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_SHADER_MODEL, &shaderModel, sizeof(D3D12_FEATURE_DATA_SHADER_MODEL));
	if (hr == S_OK)
	{
		m_errorLog->WriteError("CVideoDevice::CreateDevice::D3D_SHADER_MODEL::D3D_SHADER_MODEL_6_7\n");
	}
...
void CVideoDevice::DeviceInformation()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::DeviceInformation::EnumOutputs:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::DeviceInformation::GetDisplayModeList for numModes:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::DeviceInformation::GetDisplayModeList:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
	...
	for (UINT i = 0; i < m_numModes; i++)
	{
		...
		{
			m_errorLog->WriteError("CVideoDevice::DeviceInformation::w:%i h:%i h:%i\n", m_displayModeList[i].Width, m_displayModeList[i].Height, m_displayModeList[i].RefreshRate.Numerator / m_displayModeList[i].RefreshRate.Denominator);
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::DXGIFactory::CheckFeatureSupport:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
...
void CVideoDevice::CreateCommandQueue()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CommandQueue::CreateCommandQueue:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
}

void CVideoDevice::CreateSwapChain()
{
	m_swapChainDesc.BufferCount = CVideoDevice::BACKBUFFER_COUNT;
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CreateSwapChain::CreateSwapChain:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
...
ComPtr<ID3D12CommandAllocator> CVideoDevice::CreateCommandAllocator()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CommandProcessor::CreateCommandAllocator:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
ComPtr<ID3D12GraphicsCommandList> CVideoDevice::CreateCommandList(ComPtr<ID3D12CommandAllocator> ca)
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CreateCommandList::CreateCommandList:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
void CVideoDevice::CreateFence()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CVideoDevice::CreateFence::CreateFence:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
void CVideoDevice::ResetAndClearTargets()
{
	...
	m_commandList->ClearRenderTargetView(m_swapChainRenderTargets[m_backbufferIndex]->m_handle, CornflowerBlue.GetFloatArray(), 0, nullptr);
...
void CVideoDevice::MoveToNextFrame()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("MoveToNextFrame::commandQueue->Signal:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
	...
	if (m_fence->GetCompletedValue() < m_fenceValues[m_backbufferIndex])
	{
		...
		if (hr < 0)
		{
			m_errorLog->WriteError("MoveToNextFrame::fence->SetEventOnCompletion:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
		}
...
void CVideoDevice::ToggleFullscreenWindow()
{
	if (m_window->m_windowed)
	{
		...
		SetWindowPos(m_window->m_hWnd, HWND_TOPMOST, m_fullscreenWindowRect.left, m_fullscreenWindowRect.top, m_fullscreenWindowRect.right, m_fullscreenWindowRect.bottom, SWP_FRAMECHANGED | SWP_NOACTIVATE);
	...
	else
	{
		...
		SetWindowPos(m_window->m_hWnd, HWND_NOTOPMOST, m_windowRect.left, m_windowRect.top, m_windowRect.right - m_windowRect.left, m_windowRect.bottom - m_windowRect.top, SWP_FRAMECHANGED | SWP_NOACTIVATE);
