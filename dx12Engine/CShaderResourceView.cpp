...
void CShaderResourceView::SlotBuffer(D3D12_CPU_DESCRIPTOR_HANDLE handle, UINT64 slot, CTexture* texture)
{
	...
	handle.ptr += (m_slot * m_globalObjects->m_videoDevice->m_device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE::D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
	...
	texture->m_texture->SetName(MBSToWide(texture->m_name->GetText()));
}
