...
CDepthBuffer::CDepthBuffer(CErrorLog* errorLog, ComPtr<ID3D12Device> device, UINT width, UINT height)
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CDepthBuffer::CDepthBuffer::CreateDescriptorHeap:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	m_desc.Flags = D3D12_RESOURCE_FLAGS::D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL | D3D12_RESOURCE_FLAGS::D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE;
	...
	hr = m_device->CreateCommittedResource(&m_heapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_desc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_DEPTH_WRITE, &m_clearValue, IID_PPV_ARGS(&m_resource));

	if (hr < 0)
	{
		m_errorLog->WriteError("CDepthBuffer::CDepthBuffer::CreateCommittedResource:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
