...
CRootSignature::CRootSignature(CGlobalObjects* globalObjects, UINT numDescriptorRanges, CRange* range)
{
	...
	HRESULT hr = m_globalObjects->m_videoDevice->m_device->CheckFeatureSupport(D3D12_FEATURE::D3D12_FEATURE_ROOT_SIGNATURE, &m_featureData, sizeof(D3D12_FEATURE_DATA_ROOT_SIGNATURE));
	...
	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CRootSignature::CRootSignature::D3D12SerializeVersionedRootSignature:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	hr = m_globalObjects->m_videoDevice->m_device->CreateRootSignature(0, m_tempSignature->GetBufferPointer(), m_tempSignature->GetBufferSize(), IID_PPV_ARGS(&m_signature));

	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CRootSignature::CRootSignature::CreateRootSignature:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
