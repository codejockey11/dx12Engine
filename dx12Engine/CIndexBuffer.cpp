...
BYTE CIndexBuffer::CreateResource()
{
	...
	HRESULT hr = m_globalObjects->m_videoDevice->m_device->CreateCommittedResource(&m_heapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_resourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_indexBuffer));

	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CIndexBuffer::CreateResource::CreateCommittedResource:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
