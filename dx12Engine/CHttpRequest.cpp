	...
	if (m_curl)
	{
		...
		if (fields)
		{
			...
			while (lln->m_element)
			{
				...
				curl_mime_name(field, lln->m_element->m_name->GetText());
				curl_mime_data(field, lln->m_element->m_data->GetText(), lln->m_element->m_data->GetLength());
...
void CHttpRequest::DumpBuffers()
{
	...
	while (lln->m_element)
	{
		m_globalObjects->m_errorLog->WriteBytes(lln->m_element->GetText());
