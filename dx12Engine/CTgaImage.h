...
public:
	...
	char m_signature[CTgaFooter::SIGNATURE_SIZE];		/* TGA Signature */
	...
	void ReadValues(FILE* file)
	{
		...
		fread_s(&m_signature, sizeof(char) * CTgaFooter::SIGNATURE_SIZE, sizeof(char), CTgaFooter::SIGNATURE_SIZE, file);
	}
};
...
public:
	...
	{
		AUTHORNAME_SIZE = 41,
		AUTHORCOMMENT_SIZE = 324,
		JOBNAME_SIZE = 41,
		SOFTWAREID_SIZE = 41
	};

	WORD  m_size;                   /* Extension Size */
	char  m_authorName[CTgaExtension::AUTHORNAME_SIZE];         /* Author Name */
	char  m_authorComment[CTgaExtension::AUTHORCOMMENT_SIZE];     /* Author Comment */
	...
	char  m_jobName[CTgaExtension::JOBNAME_SIZE];            /* Job Name/ID */
	WORD  m_jobHour;                /* Job Time: Hours */
	WORD  m_jobMinute;              /* Job Time: Minutes */
	WORD  m_jobSecond;              /* Job Time: Seconds */
	char  m_softwareId[CTgaExtension::SOFTWAREID_SIZE];         /* Software ID */
...
public:
	...
	BGR m_colorTable[CTgaImage::COLORTABLE_SIZE];

	BYTE m_imageDescription[CTgaImage::IMAGEDESCRIPTION_SIZE];
...
private:
	...
	char m_text[CTgaImage::MAX_TEXT];
