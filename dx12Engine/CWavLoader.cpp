...
CWavLoader::CWavLoader(CErrorLog* errorLog, const char* filename)
{
	...
	if (err)
	{
		m_errorLog->WriteError("CWavLoader::CWavLoader::Failed:%i:%s\n", err, filename);
	...
	bytesRead = fread_s(&m_header.m_chunkID, 4 * sizeof(BYTE), sizeof(BYTE), 4, file);		//Contains the letters "RIFF" in ASCII form
	bytesRead = fread_s(&m_header.m_chunkSize, sizeof(int), sizeof(int), 1, file);
	bytesRead = fread_s(&m_header.m_format, 4 * sizeof(BYTE), sizeof(BYTE), 4, file);		//Contains the letters "WAVE"
	bytesRead = fread_s(&m_header.m_subchunk1ID, 4 * sizeof(BYTE), sizeof(BYTE), 4, file);		//Contains the letters "fmt "
	bytesRead = fread_s(&m_header.m_subchunk1Size, sizeof(int), sizeof(int), 1, file);		//16 for PCM.This is the size of the rest of the Subchunk which follows this number.
	bytesRead = fread_s(&m_header.m_audioFormat, 2 * sizeof(BYTE), sizeof(BYTE), 2, file);		//PCM = 1 (i.e.Linear quantization)
	...
	bytesRead = fread_s(&m_header.m_bitsPerSample, 2 * sizeof(BYTE), sizeof(BYTE), 2, file);		//8 bits = 8, 16 bits = 16
	...
	bytesRead = fread_s(&m_header.m_subchunk2ID, 4 * sizeof(BYTE), sizeof(BYTE), 4, file);		//Contains the letters "data"
	bytesRead = fread_s(&m_header.m_subchunk2Size, sizeof(int), sizeof(int), 1, file);			//NumSamples * NumChannels * BitsPerSample / 8 size	of the read of the subchunk following this number
