...
void CTgaImage::DumpHeader()
{
	...
	memset((void*)m_text, 0x00, CTgaImage::MAX_TEXT);
	sprintf_s(m_text, CTgaImage::MAX_TEXT, "%05d", m_header->m_width);

	fwrite(m_text, sizeof(char), strlen(m_text), file);

	memset((void*)m_text, 0x00, CTgaImage::MAX_TEXT);
	sprintf_s(m_text, CTgaImage::MAX_TEXT, "%05d", m_header->m_height);
...
void CTgaImage::LoadImageDescription()
{
	...
	{
		m_err = fread_s(&m_imageDescription, sizeof(BYTE) * CTgaImage::IMAGEDESCRIPTION_SIZE, sizeof(BYTE), m_header->m_idLength, m_file);
	}
}
...
void CTgaImage::LoadColorTable()
{
	...
	{
		m_err = fread_s(&m_colorTable, sizeof(BGR) * CTgaImage::COLORTABLE_SIZE, sizeof(BGR), m_header->m_cMapLength, m_file);
	}
}
