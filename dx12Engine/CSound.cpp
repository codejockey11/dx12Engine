...
CSound::CSound(CErrorLog* errorLog, CSoundDevice* soundDevice, const char* filename, bool loop)
{
	...
	for (UINT i = 0; i < CSound::SOURCE_VOICE_COUNT; i++)
	{
	...
	HRESULT hr = XAudio2CreateReverb(&m_xapoEffect[CSound::SE_REVERB]);
	...
	hr = CreateFX(__uuidof(FXEcho), &m_xapoEffect[CSound::SE_ECHO]);
	...
	hr = CreateFX(__uuidof(FXReverb), &m_xapoEffect[CSound::SE_REVERBXAPO]);
	...
	m_effectDesc[CSound::SE_REVERB].InitialState = false;
	m_effectDesc[CSound::SE_REVERB].OutputChannels = m_wavLoader->m_wfx.nChannels;
	m_effectDesc[CSound::SE_REVERB].pEffect = m_xapoEffect[CSound::SE_REVERB];

	m_effectDesc[CSound::SE_ECHO].InitialState = false;
	m_effectDesc[CSound::SE_ECHO].OutputChannels = m_wavLoader->m_wfx.nChannels;
	m_effectDesc[CSound::SE_ECHO].pEffect = m_xapoEffect[CSound::SE_ECHO];

	m_effectDesc[CSound::SE_REVERBXAPO].InitialState = false;
	m_effectDesc[CSound::SE_REVERBXAPO].OutputChannels = m_wavLoader->m_wfx.nChannels;
	m_effectDesc[CSound::SE_REVERBXAPO].pEffect = m_xapoEffect[CSound::SE_REVERBXAPO];


	m_effectChain.EffectCount = CSound::EFFECT_COUNT;
...
CSound::~CSound()
{
	...
	if (m_xapoEffect[CSound::SE_REVERB])
	{
		m_xapoEffect[CSound::SE_REVERB]->Release();
	}

	if (m_xapoEffect[CSound::SE_ECHO])
	{
		m_xapoEffect[CSound::SE_ECHO]->Release();
	}

	if (m_xapoEffect[CSound::SE_REVERBXAPO])
	{
		m_xapoEffect[CSound::SE_REVERBXAPO]->Release();
	}
}
...
void CSound::SetEffectParameters()
{
	for (UINT i = 0; i < CSound::SOURCE_VOICE_COUNT; i++)
	{
		HRESULT hr = m_sourceVoice[i]->SetEffectParameters(CSound::SE_REVERB, &m_reverbParameters, sizeof(XAUDIO2FX_REVERB_PARAMETERS), XAUDIO2_COMMIT_NOW);
		...
		hr = m_sourceVoice[i]->SetEffectParameters(CSound::SE_ECHO, &m_xapoEcho, sizeof(FXECHO_PARAMETERS), XAUDIO2_COMMIT_NOW);
		...
		hr = m_sourceVoice[i]->SetEffectParameters(CSound::SE_REVERBXAPO, &m_xapoReverb, sizeof(FXREVERB_PARAMETERS), XAUDIO2_COMMIT_NOW);
