...
CD12Font::CD12Font(CErrorLog* errorLog, CVideoDevice* videoDevice, CWindow* window, const char* face, float pitch, DWRITE_FONT_WEIGHT weight)
{
	memset(this, 0x00, sizeof(CD12Font));
	...
	HRESULT hr = m_videoDevice->m_d3d11On12->m_dWriteFactory->CreateTextFormat(MBSToWide(face), nullptr, weight, DWRITE_FONT_STYLE::DWRITE_FONT_STYLE_NORMAL, DWRITE_FONT_STRETCH::DWRITE_FONT_STRETCH_NORMAL, pitch, localName, &m_textFormat);

	if (hr < 0)
	{
		m_errorLog->WriteError("CD12Font::CD12Font::CreateTextFormat:%s:%s\n", face, m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	hr = m_videoDevice->m_d3d11On12->m_d2dDeviceContext->CreateSolidColorBrush(D2D1::ColorF(White.m_RGBA[0], White.m_RGBA[1], White.m_RGBA[2], White.m_RGBA[3]), &m_textBrush);

	if (hr < 0)
	{
		m_errorLog->WriteError("CD12Font::CD12Font::CreateSolidColorBrush:%s:%s\n", face, m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	}
}
...
void CD12Font::Draw(const char* text, CVertex2 position, CVertex2 size, CColor* color)
{
	...
	{
		m_videoDevice->m_d3d11On12->m_d2dDeviceContext->DrawText(MBSToWide(text), (UINT32)strlen(text), m_textFormat.Get(), &m_textRect, m_textBrush.Get());
	}
}
