...
CConstantBuffer::CConstantBuffer(CGlobalObjects* globalObjects, ComPtr<ID3D12GraphicsCommandList> commandList, UINT bufferSize)
{
	...
	HRESULT hr = m_globalObjects->m_videoDevice->m_device->CreateCommittedResource(&m_bufferHeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_bufferDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, nullptr, IID_PPV_ARGS(&m_buffer));

	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CConstantBuffer::CConstantBuffer::CreateCommittedResource::buffer:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	hr = m_globalObjects->m_videoDevice->m_device->CreateCommittedResource(&m_heapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_heapDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_uploadHeap));

	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CConstantBuffer::CConstantBuffer::CreateCommittedResource::uploadHeap:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
void CConstantBuffer::UpdateBuffer(void* values)
{
	...
	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CConstantBuffer::UpdateBuffer::uploadHeap->Map:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
