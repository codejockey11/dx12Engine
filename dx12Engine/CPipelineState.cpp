...
void CPipelineState::Create(bool alphaToCoverageEnable, bool blendEnable, bool depthEnable, D3D12_PRIMITIVE_TOPOLOGY_TYPE primitiveType, CShader* vertexShader, CShader* pixelShader, CShader* domainShader, CShader* hullShader, CShader* geometryShader, ComPtr<ID3D12RootSignature> rootSignature)
{
	...
	if (hr < 0)
	{
		m_globalObjects->m_errorLog->WriteError("CPipelineState::Create::CreateGraphicsPipelineState:%s\n", m_globalObjects->m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
