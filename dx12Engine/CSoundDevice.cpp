...
CSoundDevice::CSoundDevice(CErrorLog* errorLog)
{
	...
	m_CLSID_MMDeviceEnumerator = __uuidof(MMDeviceEnumerator);
	m_IID_IMMDeviceEnumerator = __uuidof(IMMDeviceEnumerator);

	// Get the audio devices on the system
	HRESULT hr = CoCreateInstance(m_CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, m_IID_IMMDeviceEnumerator, (void**)&m_pEnumerator);
