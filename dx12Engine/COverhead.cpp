...
COverhead::COverhead(CGlobalObjects* globalObjects, CVertex2 size, CVertex2 position)
{
	...
	for (UINT i = 0; i < CVideoDevice::BACKBUFFER_COUNT; i++)
	{
		...
		m_depthBuffers[i] = new CDepthBuffer(m_globalObjects->m_errorLog, m_globalObjects->m_videoDevice->m_device, (UINT)size.p.x, (UINT)size.p.y);
	}
	...
	m_texture = new CDynamicTexture(m_globalObjects->m_errorLog, m_globalObjects->m_videoDevice, m_commandList, 256, 256, "CDynamicTexture::Overhead");
	...
	m_pipelineState->Create(false, true, false, D3D12_PRIMITIVE_TOPOLOGY_TYPE::D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, m_vertexShader, m_pixelShader, nullptr, nullptr, nullptr, m_rootSignature->m_signature);
	...
	m_vertexBuffer = new CVertexBuffer(m_globalObjects, m_commandList, CVertex::VT_VERTEXT, 4, (void*)m_vertices);
	...
	m_camera = new CCamera(m_size.p.x, m_size.p.y, 45.0f, 1.0f, 50000.0f);
...
{
	m_globalObjects->m_videoDevice->m_commandList->ClearRenderTargetView(m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_handle, LovelyPurple.GetFloatArray(), 0, nullptr);
	m_globalObjects->m_videoDevice->m_commandList->ClearDepthStencilView(m_depthBuffers[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_handle, D3D12_CLEAR_FLAGS::D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);
}
...
void COverhead::Record()
{
	...
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.pResource = m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_resource.Get();
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.StateBefore = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RENDER_TARGET;
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.StateAfter = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_COPY_SOURCE;

	m_commandList->ResourceBarrier(1, &m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier);

	m_commandList->CopyResource(m_texture->m_texture.Get(), m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_resource.Get());
	...
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.pResource = m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_resource.Get();
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.StateBefore = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_COPY_SOURCE;
	m_renderTargets[m_globalObjects->m_videoDevice->m_backbufferIndex]->m_barrier.Transition.StateAfter = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RENDER_TARGET;
