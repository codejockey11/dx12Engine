...
CCamera::CCamera(float width, float height, float fov, float nearPlane, float farPlane)
{
	memset(this, 0x00, sizeof(CCamera));
	...
	m_xmproj = XMMatrixPerspectiveFovLH(fov * PiDiv180, width / height, nearPlane, farPlane);
...
void CCamera::UpdateView()
{
	...
	// In order to transform the memory from the format defined on the CPU to the order expected in HLSL, you need to transpose the CPU matrix before sending it to the GPU.
