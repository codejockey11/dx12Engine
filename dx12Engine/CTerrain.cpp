...
CTerrain::CTerrain(CGlobalObjects* globalObjects, const char* filename)
{
	...
	m_pipelineState->Create(false, true, true, D3D12_PRIMITIVE_TOPOLOGY_TYPE::D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE, m_vertexShader, m_pixelShader, nullptr, nullptr, nullptr, m_rootSignature->m_signature);
	...
	m_vertexBuffer = new CVertexBuffer(m_globalObjects, m_commandList, CVertex::VT_VERTEXNT, m_width * m_height, (void*)vertices);
	...
	err = fopen_s(&file, "terrain\\clips.txt", "rb");
	...
	if (file)
	{
		...
		bytesRead = fread_s(m_clips->m_heap, (size_t)(sizeof(CTerrainClip) * (size_t)(m_width - 1) * (size_t)(m_height - 1)), sizeof(CTerrainClip), (size_t)(m_width - 1) * (size_t)(m_height - 1), file);
