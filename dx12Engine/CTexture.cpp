...
void CTexture::CreateResource(BYTE* pixels)
{
	...
	HRESULT hr = m_videoDevice->m_device->CreateCommittedResource(&m_textureHeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_textureDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_texture));

	if (hr < 0)
	{
		m_errorLog->WriteError("CTexture::CreateResource::CreateCommittedResource::texture:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	hr = m_videoDevice->m_device->CreateCommittedResource(&m_heapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &m_heapDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_textureUploadHeap));

	if (hr < 0)
	{
		m_errorLog->WriteError("CTexture::CreateResource::CreateCommittedResource::textureUploadHeap:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CTexture::CreateResource::textureUploadHeap->Map:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	{
		m_errorLog->WriteError("CTexture::CreateResource::MemToAlloc:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	if (pMem == nullptr)
	{
		m_errorLog->WriteError("CTexture::CreateResource::HeapAlloc:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
CDynamicTexture::CDynamicTexture(CErrorLog* errorLog, CVideoDevice* videoDevice, ComPtr<ID3D12GraphicsCommandList> commandList, UINT width, UINT height, const char* name)
{
	memset(this, 0x00, sizeof(CTexture));
...
void CDynamicTexture::CreateResource()
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CTexture::CreateDynamicResource::CreateCommittedResource::textureUploadHeap:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
...
void CDynamicTexture::UpdateTextureResource(void* pixels)
{
	...
	if (hr < 0)
	{
		m_errorLog->WriteError("CTexture::UpdateDynamicResource::textureUploadHeap->Map:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	if (MemToAlloc > SIZE_MAX)
	{
		m_errorLog->WriteError("CTexture::UpdateDynamicResource::MemToAlloc:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
	...
	if (pMem == nullptr)
	{
		m_errorLog->WriteError("CTexture::UpdateDynamicResource::HeapAlloc:%s\n", m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText());
