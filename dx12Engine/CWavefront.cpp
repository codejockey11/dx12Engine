...
CWavefront::CWavefront(const char* filename, const char* materialname)
{
	...
	for (int i = 0; i < CMaterial::MATERIAL_COUNT; i++)
	{
	...
	if (file)
	{
		while (fscanf_s(file, "%s", line, CWavefront::LINE_LENGTH) != EOF)
		{
			if (strcmp(line, "usemtl") == 0)
			{
				...
				bytesRead = fscanf_s(file, "%s\n", &m_materialName[m_inbr], CMaterial::MATERIAL_LENGTH);
		...
		if ((m_inbr > -1) && (m_inbr < CMaterial::MATERIAL_COUNT))
		{
	...
	if (file)
	{
		while (fscanf_s(file, "%s", line, CWavefront::LINE_LENGTH) != EOF)
		{
			if (strcmp(line, "newmtl") == 0)
			{
				...
				if (mnbr == CMaterial::MATERIAL_COUNT)
				{
					fclose(file);
			...
			if (strcmp(line, "Ka") == 0)
			{
				bytesRead = fscanf_s(file, "%f %f %f\n", &m_mesh[mnbr]->m_material->m_ambient.x, &m_mesh[mnbr]->m_material->m_ambient.y, &m_mesh[mnbr]->m_material->m_ambient.z);
			}

			if (strcmp(line, "Kd") == 0)
			{
				bytesRead = fscanf_s(file, "%f %f %f\n", &m_mesh[mnbr]->m_material->m_diffuse.x, &m_mesh[mnbr]->m_material->m_diffuse.y, &m_mesh[mnbr]->m_material->m_diffuse.z);
			}

			if (strcmp(line, "Ks") == 0)
			{
				bytesRead = fscanf_s(file, "%f %f %f\n", &m_mesh[mnbr]->m_material->m_specular.x, &m_mesh[mnbr]->m_material->m_specular.y, &m_mesh[mnbr]->m_material->m_specular.z);
			}

			if (strcmp(line, "Ke") == 0)
			{
				bytesRead = fscanf_s(file, "%f %f %f\n", &m_mesh[mnbr]->m_material->m_emissive.x, &m_mesh[mnbr]->m_material->m_emissive.y, &m_mesh[mnbr]->m_material->m_emissive.z);
			}
...
void CWavefront::LoadMeshList()
{
	for (int mc = 0; mc < m_inbr + 1; mc++)
	{
		if (m_icount[mc] > 0)
		{
			...
			mesh->m_material = m_mesh[mc]->m_material;
			
			mesh->m_map_Ka = m_mesh[mc]->m_map_Ka;
			mesh->m_map_Kd = m_mesh[mc]->m_map_Kd;
			mesh->m_map_Ks = m_mesh[mc]->m_map_Ks;
			mesh->m_map_Ns = m_mesh[mc]->m_map_Ns;
			mesh->m_map_d = m_mesh[mc]->m_map_d;
			mesh->m_map_Kb = m_mesh[mc]->m_map_Kb;
			mesh->m_refl = m_mesh[mc]->m_refl;
